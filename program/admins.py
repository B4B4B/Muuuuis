...
async def update_admin(client, message):
    ...
    await message.reply_text(
        "✅تم إعادة تحميل البوت ** بشكل صحيح! **  \n✅ ** تم تحديث قائمة مشرفين ** **! ** "
    )


@Client.on_message(command(["skip", "كب","تخطي", f"skip@{BOT_USERNAME}", "vskip"]) & other_filters)
@authorized_users_only
...
@Client.on_message(
    command(["stop", "انهاء", "اغلاق", f"stop@{BOT_USERNAME}", "end", f"end@{BOT_USERNAME}", "vstop"])
    & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["pause", "وقت", "ايقاف" f"pause@{BOT_USERNAME}", "vpause"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["resume", "استمرار","تابع", f"resume@{BOT_USERNAME}", "vresume"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["mute", "كتم" , f"mute@{BOT_USERNAME}", "vmute"]) & other_filters
)
@authorized_users_only
...
async def cbstop(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        ...
        await query.answer("❌ مفيش اغنيه", show_alert=True)
